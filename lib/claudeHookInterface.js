...
export class ClaudeHookInterface {
    constructor(extension) {
        ...
        this._signalIds = [];
        // 不在constructor中执行异步初始化，由外部调用
        this._connectSignals();
    }

    /**
     * Initialize hook interface (异步初始化)
     * 必须从外部调用
     */
    async initialize() {
        try {
            await this._setupHookConfig();
     ...
     * 通知设置变化处理 (异步版本)
     */
    async _onNotificationSettingsChanged() {
        try {
            await this._setupHookConfig();
        } catch (e) {
    ...
    /**
     * Setup hook configuration in Claude settings (异步版本)
     */
    async _setupHookConfig() {
        await this._installHookConfig();
    }
        ...
        try {
            ...
            if (!dirExists) {
                await new Promise((resolve, reject) => {
                    claudeDirFile.make_directory_async(
                        GLib.PRIORITY_DEFAULT,
                        null,
                        (source, result) => {
                            try {
            ...
            // Read existing settings or create new ones (异步)
            let settings = {};
    ...
    /**
     * Get hook interface status (异步版本)
     */
    async getStatus() {
        return {
            enabled: this._isEnabled,
            hookConfigInstalled: await this._isHookConfigInstalled(),
            providerStatus: this.checkProviderStatus()
        };
    }

    /**
     * Check if hook configuration is installed (异步版本)
     */
    async _isHookConfigInstalled() {
        try {
            const homeDir = GLib.get_home_dir();
    ...
    /**
     * Remove hook configuration (异步版本)
     * Only removes our hooks, preserves custom hooks
     */
    async uninstallHookConfig() {
        try {
            const homeDir = GLib.get_home_dir();
            const settingsFile = GLib.build_filenamev([homeDir, '.claude', 'settings.json']);
            const file = Gio.File.new_for_path(settingsFile);
            ...
            try {
                ...
                if (!contents) {
                    return true; // File doesn't exist, nothing to uninstall
                }
                ...
                console.error('Failed to read or parse settings file:', e);
                return false;
            }
        } catch (e) {
